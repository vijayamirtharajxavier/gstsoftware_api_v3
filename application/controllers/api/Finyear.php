...
class Finyear extends REST_Controller {
    ...
    */
    ...
    public function finyearsettings_get($id = 0)
	{
        if(!empty($id)){
            $data = $this->db->get_where("settings_tbl", ['company_id' => $id])->row_array();
        }else{
            $data = $this->db->order_by("finyear", "desc")->get("settings_tbl")->result();
        }
