    ...
    */
	public function index_get($finyear = null,$cid=null)
	{
        if(!empty($finyear)){
            ...
            $data = $this->db->get_where("settings_tbl", ['finyear' => $finyear,'company_id'=>$cid])->result();

//            $data = $this->db->get_where("settings_tbl", ['finyear'=> $finyear])->get("settings_tbl")->result();
        }
     
        $this->response($data, REST_Controller::HTTP_OK);
	}
	public function settingsbycid_get($cid=null)
	{
        if(!empty($finyear)){
            $data = $this->db->get_where("settings_tbl", ['company_id'=>$cid])->row_array();
        }else{
            $data = $this->db->get_where("settings_tbl", ['company_id'=>$cid])->row_array();
