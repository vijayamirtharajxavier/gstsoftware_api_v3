...
class Product extends REST_Controller {
    ...
    */
     ...
     * @return Response
    */
	public function index_get($compid=null)
	{
$data=array();
  //      $data[] = $this->db->get_where("products_tbl", ['company_id'=> $compid])->row_array();
    $sql="SELECT * FROM products_tbl where company_id=?";
    $query = $this->db->query($sql, array($compid));
    return $query->result_array();
      


// $result[]=        $this->response($data, REST_Controller::HTTP_OK);
//echo json_encode($data);	
}
      
	public function productbyid_get($id = 0,$compid=null)
	{
            $data = $this->db->get_where("products_tbl", ['id' => $id,'company_id'=> $compid])->row_array();
