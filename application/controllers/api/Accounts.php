...
//var_dump($finyear);
        $this->db->order_by('id','desc');
            $data = $this->db->get_where("transaction_tbl", ['trans_type' => $trans_type,'finyear'=>$finyear,'company_id'=>$compid])->result();
    ...
    public function index_put($id)
    {
     ...
     //  var_dump($input);
      $status=$this->db->update('transaction_tbl', $input, array('id'=>$id));
             $msg= array("status"=>$status,"success"=>true,"messages"=>"updated successfully");
       // $this->response($msg, REST_Controller::HTTP_OK);
